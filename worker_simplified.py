"""
Simplified article worker that delegates chunking, FTS, and embeddings to separate services
"""
...
class SimplifiedArticleWorker:
    """Simplified article worker that focuses only on content extraction and parsing"""
    ...
    def process_pending_articles(self) -> Dict[str, Any]:
        ...
        Only handles content extraction and parsing - no chunking/FTS/embeddings
        """
        logger.info("Starting simplified article processing")
        ...
        self.db.log_diagnostics(
            level='INFO',
            component='simplified_worker',
            message=f"Processed {stats['articles_processed']} articles (no chunking/FTS/embeddings)",
    ...
    async def _process_single_article(self, article: Dict[str, Any]) -> Dict[str, Any]:
        """Process a single article - content extraction only"""
        result = {
            'status': 'error',
            'error': None
        }
        ...
        try:
            ...
            # Add to articles index for downstream processing
            if parsed_article.status == 'stored' and parsed_article.text_hash:
                ...
                index_data = {
                    ...
                    'source': parsed_article.source,
                    # extended fields for downstream services
                ...
                try:
                    ...
                    logger.info(f"Article indexed for downstream processing: {article_id}")
                except Exception as e:
...
def process_pending_simplified(db_client, worker_id: str = "simplified-worker-1", batch_size: int = 50):
    """Legacy function wrapper for the simplified worker"""
    worker = SimplifiedArticleWorker(db_client, batch_size=batch_size)
    try:
